...
var itemListStore = new Ext.data.JsonStore({
	root : 'ResultSet["Result"]["Item"]',
	totalProperty : 'ResultSet["@attributes"]["totalResultsAvailable"]',
	idProperty : 'threadid',
	id : 'itemListStore',
	remoteSort : true,
	fields : ['auctionID', 'Title',{name:'Seller',mapping:'Seller.Id'}, 'Id','ItemUrl','Image'],
	/*
	...
	*/
	proxy:new Ext.data.ScriptTagProxy({
    	...
    	//url: 'http://auctions.yahooapis.jp/AuctionWebService/V2/json/search?appid=1hyQM_.xg64UN1RnE406vvWL2JJQK1iqB57THqGXVNPTLIXK7HY.zXvrsntRkQM-&query=ipod&callback=load'
	}),
	paramNames : {
		start : 'START_INDEX',
		limit : 'PAGE_SIZE',
...
itemListStore.load(createLoadBidItemsParams(itemListStore, 0, 50, '', '',loadBidItemsData));
//	  itemListStore.load();
	// alert('hihihihi');
	return grid;
};
...
itemImgCol=function (value,cellmeta,record,rowIndex,columnIndex,store){
	return "<img src='"+record['data']['Image']+"' />";
//	return "<input type='button' value='上刊' onclick=\"alert('"+record['data']['item_order_id']+"');\" />";
}
/**
 * 案件詳細資訊
