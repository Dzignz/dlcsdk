Ext.namespace("Mogan.netAgent.bid");

// UI介面相關
Mogan.netAgent.bid.createAccountFrom = function() {// 帳號管理表單
	...
	var simple = new Ext.FormPanel({
				...
				buttons : [{
							id : 'btnLogin',
							handler : Mogan.netAgent.bid.login,
							text : '登入'
							...
							text : '顯示登入列表',
							handler : Mogan.netAgent.bid.getLoginList
						}, {
							text : '顯示登入列表(all)',
							handler : Mogan.netAgent.bid.getLoginListAll
						}]
			});
...
Mogan.netAgent.bid.createBidFrom = function() {// 下標用表單
	var accountData = [];
	...
	var simple = new Ext.FormPanel({
				...
				buttons : [{
							...
							text : '出價',
							handler : function() {
								Mogan.netAgent.bid.bidItem('BID_ITEM');
							}
						}, {
							text : '直購',
							handler : function() {
								Mogan.netAgent.bid.bidItem('BUY_ITEM')
							}
						}, {
							text : '自動出價到最高標'
						}, {
							text : '是否得標',
							handler : Mogan.netAgent.bid.isMyBid
						}, {
							text : '是否得標(出價履歷)',
							handler : Mogan.netAgent.bid.checkBidHistory
						}]
			});
	return simple;
};

Mogan.netAgent.bid.createMsgArea = function() {// 訊息區
...
Mogan.netAgent.bid.createStatusBar = function() {// 底部狀態列
...
// 登入
Mogan.netAgent.bid.login = function() {
...
// 下標
Mogan.netAgent.bid.bidItem = function(act) {
	var comboxLoginAccount = Ext.getCmp("comboxLoginAccount");
...
Mogan.netAgent.bid.checkBidHistory = function() {
	var comboxLoginAccount = Ext.getCmp("comboxLoginAccount");
	var bidAccount = comboxLoginAccount.value;
...
Mogan.netAgent.bid.isMyBid = function() {
	var comboxLoginAccount = Ext.getCmp("comboxLoginAccount");
	var bidAccount = comboxLoginAccount.value;
...
// 取得已登入列表，指定APP_ID
Mogan.netAgent.bid.getLoginList = function() {
...
// 取得已登入列表，不指定APP_ID
Mogan.netAgent.bid.getLoginListAll = function() {
	setStatusMsg(" 取得登入帳號清單中....");
	Ext.Ajax.request({
				url : 'AjaxPortal',
				callback : function() {
