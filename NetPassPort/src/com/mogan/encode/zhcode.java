...
/*
 * Copyright 2002 Erik Peterson Code and program free for non-commercial use. Contact erik@mandarintools.com for fees and licenses for commercial use.
 */

public class zhcode extends Encoding {
	// Simplfied/Traditional character equivalence hashes
	private static Logger logger = Logger.getLogger(zhcode.class.getName() );
	protected Hashtable s2thash, t2shash;
	private SinoDetect sinodetector; // Dian mark 功能不明
	...
	public String convertString(String inline, int source_encoding,
		...
		/*
		 * int lineindex; String currchar; CharSequence dataline; if (source_encoding == HZ) { dataline = hz2gb(inline.toString()); } else { dataline = inline; } for (lineindex = 0; lineindex < dataline.length(); lineindex++) { currchar = "" + dataline.charAt(lineindex); if ((source_encoding == GB2312 || source_encoding == GBK || source_encoding == ISO2022CN_GB || source_encoding == HZ || source_encoding == UNICODE || source_encoding == UNICODES || source_encoding == UTF8 || source_encoding == UTF8S) && (target_encoding == BIG5 || target_encoding == CNS11643 || target_encoding == UNICODET || target_encoding == UTF8T || target_encoding == ISO2022CN_CNS)) { if (s2thash.containsKey(currchar) == true) { outline.append(s2thash.get(currchar)); } else { outline.append(currchar); } } else if ((source_encoding == BIG5 || source_encoding == CNS11643 || source_encoding == UNICODET || source_encoding == UTF8 || source_encoding == UTF8T || source_encoding == ISO2022CN_CNS || source_encoding == GBK || source_encoding == UNICODE) && (target_encoding == GB2312 || target_encoding == UNICODES || target_encoding == ISO2022CN_GB || target_encoding == UTF8S || target_encoding == HZ)) { if (t2shash.containsKey(currchar) == true) { outline.append(t2shash.get(currchar)); } else { outline.append(currchar); } } else { outline.append(currchar); } } if (target_encoding == HZ) { // Convert to look like HZ return gb2hz(outline.toString()); } return outline.toString();
		 */
	}
	...
	/*
	 * public void convertFile(String sourcefile, String outfile, int source_encoding, int target_encoding) { BufferedReader srcbuffer; BufferedWriter outbuffer; String dataline; try { srcbuffer = new BufferedReader(new InputStreamReader(new FileInputStream(sourcefile), javaname[source_encoding])); outbuffer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outfile), javaname[target_encoding])); while ((dataline = srcbuffer.readLine()) != null) { outbuffer.write(convertString(dataline, source_encoding, target_encoding)); outbuffer.newLine(); } srcbuffer.close(); outbuffer.close(); } catch (Exception ex) { System.err.println(ex); } }
	 */
	...
	public static void main(String argc[]) {
		...
		for (int i = 0; i < 2; i++) {
			...
			// Print Help
			if (codetype == 'h') {
				logger.info("Usage:  java zhcode -[gbc867ui2nkhra][gbc867uts2nkhr] in_file out_file");
				logger.info("  g = GB2312, b = Big5, c = CNS11643, 8 = UTF-8, 6 = UTF-8 (simp),");
				logger.info("  7 = UTF-8 (trad), u = Unicode, t = Unicode (traditional characters),");
				logger.info("  s = Unicode (simplified characters),");
				logger.info("  3 = GB18030,");
				logger.info("  i = ISO-2022-CN, 2 = ISO-2022-CN-GB, n = ISO-2022-CN-CNS,");
				logger.info("  k = GBK, h = HZ, r = EUC-KR, a = autodetect source encoding");
				System.exit(0);
			}
